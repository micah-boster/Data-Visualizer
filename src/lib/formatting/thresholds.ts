 ...
 * Values are percentage points (data is 0-100 range).
 */
export const COLUMN_THRESHOLDS: Record<string, ThresholdConfig> = {
  PENETRATION_RATE_POSSIBLE_AND_CONFIRMED: {
    ...
    high: { value: 50, reason: 'Penetration rate above 50% — unusually high' },
  },
  PENETRATION_RATE_CONFIRMED_ONLY: {
    low: { value: 5, reason: 'Confirmed penetration rate below 5% threshold' },
    high: { value: 50, reason: 'Confirmed penetration rate above 50% — unusually high' },
  },
  OUTBOUND_SMS_OPEN_RATE_FROM_DELIVERED: {
    low: { value: 10, reason: 'SMS open rate below 10% threshold' },
    high: { value: 60, reason: 'SMS open rate above 60% — unusually high' },
  },
  OUTBOUND_EMAIL_OPEN_RATE_FROM_DELIVERED: {
    low: { value: 5, reason: 'Email open rate below 5% threshold' },
    high: { value: 40, reason: 'Email open rate above 40% — unusually high' },
  },
  RAITO_FIRST_TIME_CONVERTED_ACCOUNTS: {
    low: { value: 3, reason: 'First-time conversion rate below 3% threshold' },
    high: { value: 25, reason: 'First-time conversion rate above 25% — unusually high' },
  },
  OUTBOUND_SMS_CLICK_RATE_FROM_OPENED: {
    low: { value: 2, reason: 'SMS click rate below 2% threshold' },
    high: { value: 30, reason: 'SMS click rate above 30% — unusually high' },
  },
  OUTBOUND_EMAIL_CLICK_RATE_FROM_OPENED: {
    low: { value: 1, reason: 'Email click rate below 1% threshold' },
    high: { value: 20, reason: 'Email click rate above 20% — unusually high' },
  },
  OUTBOUND_PHONE_VERIFY_RATE_FROM_ANSWERED: {
    low: { value: 10, reason: 'Outbound phone verify rate below 10% threshold' },
    high: { value: 50, reason: 'Outbound phone verify rate above 50% — unusually high' },
  },
  INBOUND_PHONE_VERIFY_RATE_FROM_ANSWERED: {
    low: { value: 10, reason: 'Inbound phone verify rate below 10% threshold' },
    high: { value: 50, reason: 'Inbound phone verify rate above 50% — unusually high' },
  },
};
